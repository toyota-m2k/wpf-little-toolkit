...
namespace io.github.toyota32k.toolkit.utils {
    public class WinPlacement
    {
        ...
        public static Rect? GetWindowPositionAtPoint(Point pos)
        {
            var hwnd = WindowFromPoint(new POINT((int)pos.X, (int)pos.Y));
            if (hwnd == IntPtr.Zero) return null;
        ...
        public static bool SetWindowPositionAtPoint(Point pos, Rect rc)
        {
            ...
            if (p != IntPtr.Zero)
            {
                SetWindowPos(p, IntPtr.Zero, (int)rc.Left, (int)rc.Top, (int)rc.Width, (int)rc.Height, flags);
                return true;
            }
            return false;
        }
